...
## 2026-09-24 — 管理端文章体验反馈批次

- 文章管理列表固定按 `DRAFT → SCHEDULED → PUBLISHED → ARCHIVED` 分组，再按所选时间字段
  和 `id DESC` 稳定排序；公开端排序保持原行为。
- 文章编辑器分类改为可搜索下拉多选面板，完整显示层级名称并取消分类分页；标签分页保持不变。
- 已有文章每 30 秒自动保存完整表单，成功、失败重试和校验暂停都显示状态；采用最后提交覆盖，
  新建文章仍只在用户提交时创建。
- 点击文章行除复选框、链接和按钮外的区域可进入编辑页，原有操作保持独立。
- AI 结果源码和预览统一由同一字符串状态驱动；超长结果返回后聚焦结果区并重置滚动位置，
  修复源码区域空白。
- 更新 Windows 视觉基线；Linux 28 个原有快照中仅受影响页面重新生成。
- 已发布正式版 `v1.1.3`：提交 `0920b0a1b87e`，push CI run `35960680216`；
  发布前备份、独立 `Verify`、ECS 31/31、公网首页和站点 API 均通过。
- 当前生产版本为 `v1.1.3`，正式回滚目标为 `v1.1.2`；开发机保留这两个版本归档。
