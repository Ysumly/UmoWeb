...
## 2026-09-20 — 修复生产 `STRUCTURE_CLEANUP` 校验策略

- 根因是模式版本 2 的提示词允许语言优化，但校验策略仍为 `EXACT_CONTENT`，模型修改措辞后
  被本地校验拒绝并显示为 502「AI 服务返回无效响应」。
- 通过管理 API 保留原提示词并创建版本 3，将校验策略改为 `NONE`；版本 1 和 2 历史保留，
  可继续查询或回滚。
- 原最小复现请求 `8f3ce1e2-2f9e-4a75-ba7a-1a46a1d4bfd7` 曾返回“结构整理改变了正文”；
  修复后同一输入请求 `6a22035e-7328-4236-a78e-d8f48adf86dc` 返回 200，
  `modeVersion=3` 且日志为 `status=success`。
- 本次为生产模式目录数据修复，不修改代码、API、Schema 或 `v1.1.2` 发布镜像。
...
- 管理端新建文章从中文标题生成无声调拼音 slug；标题持续同步，手动修改后锁定，清空后恢复，
  编辑已有文章和 Markdown 导入自带的 slug 不会被覆盖。
- 700px 及以下文章详情把篇章信息、难度、分类、章节和书目压缩为一行可展开区；桌面侧栏、
  阅读进度和目录入口保持不变。
- 移动端书库改为 `筛选（已选 N 项）` 按钮、当前筛选 chips 和草稿式底部抽屉；确认后一次更新
  URL 和结果，关闭丢弃草稿，清除筛选保留滚动位置，并补齐焦点圈定、Escape、焦点恢复和背景滚动锁定。
- 书库筛选改为同路由原地更新，`701px` 以上分类侧栏保持原布局。
- 验证：前端 Node 137/137、Windows Playwright 126/126（92 functional + 34 visual）和生产构建通过。
- Linux 视觉基线工作流 `35442006114` 生成并复核两张移动端快照，其余 32 张哈希保持不变。

## 2026-09-19 — 生产启用管理端 AI

- 在当前 ECS 将 `APP_AI_ENABLED` 从 `false` 切换为 `true` 并重启后端；
  DeepSeek Base URL、API Key 和模型继续只保存在服务器侧。
- 启用五个经过质量验收的默认转换模式：结构整理、现代文转文言文、英译中、中译英和轻度小说化。
- capability 返回 `enabled=true`、五个模式且输入上限为 20,000；三容器保持健康，
  AI 开启后 ECS 兼容冒烟继续为 31/31。

## 2026-09-19 — 管理端 AI 正式版 `v1.1.0`

- 发布同一提交 `bba9e44dc640` 的 `v1.1.0-rc.1` 和 `v1.1.0`，push CI run `35431353455`
  五个 job 全部成功；独立 `Verify` 返回目标版本、manifest 双镜像 ID 和 `operation=deploy`。
- 迁移前备份 `20260919T081631Z` 通过外层 SHA-256
  `98ab9f0130a850a126fdf3b35906cf596002ecea16328660a7bd63c8a0a56271` 和内部清单校验。
- 关闭态 RC 与正式版均完成 ECS 31/31；capability 返回 `enabled=false`、模式数 0，
  文章编辑、发布、搜索和公开端保持原有行为。
- 受控启用 `STRUCTURE_CLEANUP` 后完成一次真实转换，模式版本保持 1，
  Token 使用 `102/906/1008`，耗时 `5342ms`，结构与原文完全一致；未发现正文、结果、
  提示词或 API Key 进入后端日志。
- 演练后恢复 `APP_AI_ENABLED=false` 和模式停用；正式版保持 AI 默认关闭。开发机保留
  `v1.0.0`、`v1.1.0-rc.1` 和 `v1.1.0` 三个归档，正式回滚目标为 `v1.0.0`。

## 2026-09-19 — 5.1F AI 质量验收

- 增加仅监听回环地址的 OpenAI-compatible 假供应商，CI 使用它验证模式 CRUD、版本冲突、回滚、
  能力查询和转换协议，不读取真实模型密钥。
- AI 接口冒烟扩展为显式 `--include-ai` / `-IncludeAI` 模式，默认兼容链路保持 31/31，
  启用假供应商后覆盖 40/40 和 9 个 AI 管理接口。
- 新增五种模式的 10 个公开安全固定样本、人工评分口径和受控评测脚本；真实模型评测
  10/10 通过，全部评分为 5/5，结构样本正文一致。
- 验证：后端 238 个测试且 0 失败/18 跳过，MySQL 8.4 门控通过，40/40、77 次 HTTP 请求
  通过；前端 Node 126/126、Windows Playwright 120/120 和生产构建通过。
...
- 管理端文章编辑器保持默认同步并移除开关，改为在编辑器标题测量位置与预览标题位置之间连续插值；
  首尾补齐起点和最大滚动位置，滚动时用二分查找定位相邻标题区间，不再在标题区间内静止或跨标题跳动。
- 双向同步继续在 `requestAnimationFrame` 中每帧更新，不使用 CSS `scroll-behavior: smooth`；
  标题解析、镜像测量节点和预览标题位置按内容及宽度缓存，无标题或标题数不一致时回退整体比例。
- 更新 Windows 与 Linux 的 AI 设置和 AI 抽屉桌面/移动视觉基线。
- 本机无缓存全栈用 171,982 字符（UTF-8 261,610 字节）文章实测：相邻标题五等分采样严格单调，
  标题边界误差约 0.1–0.3px；连续 90 帧平均间隔 11.14ms、P95 12.6ms、最大 16.7ms。
- 验证：前端 Node 126/126、Windows Playwright 120/120（86 functional + 34 visual）和生产构建通过。
