...
正文索引、摘要提取、图片清理、图片一致性、管理端状态分组排序、AI 模式目录、DeepSeek Provider、
AI 单并发和结果保真校验测试。
其中 19 个真实 MySQL 测试由 `MYSQL_INTEGRATION=true` 启用，本地默认跳过；MockMvc 边界测试
...
- Vite 8.1.0 前端生产构建成功。
- 前端 139 个 Node 测试通过，覆盖路由、管理路径、主题、访问隐私配置、游戏规则与旧成绩解析、
  管理端文章/分类/标签/图片/站点/改密规则、编辑器草稿与文件规则、Markdown front matter 导入、
  API 错误解析、日期格式、书库后代参数与筛选 chips、标题拼音 slug、文章目录树/展开状态、
  标题 ID 与旧锚点兼容、AI 模式表单与
  AI 抽屉本地状态/字符边界、Markdown 安全、邻接正文的加粗、编辑器双向滚动比例、标题锚点插值
  和管理端自动保存状态转换。
- Playwright 134 个浏览器检查，其中 98 个 functional 用例覆盖公开端、文章目录、阅读进度与相关阅读、
  移动端篇章信息压缩、书库筛选抽屉、slug 自动生成、
  图片一致性检查、隐私说明、工具中心、在线编辑器与三处编辑工作区滚动协同、
  四款游戏（含高密度网格、长数字、10 张牌、旧成绩和响应式场景）、AI 模式卡片与策略说明、
  AI 抽屉长结果与模式控件、管理端状态排序、分类多选、行点击和 30 秒自动保存，
  以及标题区间连续同步与缓存失效、移动菜单焦点与横屏滚动、隐藏文件输入；
  36 个视觉断言覆盖 18 个核心页面状态的桌面与 390px 基线。
...
非法 YAML 不覆盖当前表单、创建 payload 落库以及重复 slug 留在编辑器修正。新建文章还会验证
中文标题转拼音、手改后停止同步和编辑页清空 slug 不自动生成。导入器只预填现有表单，
...
5. functional 项目覆盖公开阅读、在线编辑器、管理端认证、Markdown 导入与 CRUD、AI 模式设置、
   AI 转换抽屉，以及 390px 布局。
6. visual-desktop 和 visual-mobile 项目比较 36 张页面截图。
...
视觉基线位于 `e2e/visual.spec.js-snapshots/`，当前包含 36 张 `win32` 和 36 张 `linux`
...
2026-09-19 管理端 AI 正式版 `v1.1.0` 发布：

- `v1.1.0-rc.1` 与 `v1.1.0` 从提交 `bba9e44dc640` 构建，push CI run 为 `35431353455`。
- 迁移前备份 SHA-256 为
  `98ab9f0130a850a126fdf3b35906cf596002ecea16328660a7bd63c8a0a56271`。
- 关闭态 RC 与正式版均通过独立 `Verify`、ECS 31/31 和 capability `enabled=false` 检查。
- 受控启用后的真实转换模式版本保持 1，Token 使用为 `102/906/1008`，
  结构与原文完全一致；关闭后再次完成 31/31。
- 开发机保留 v1.0.0、v1.1.0-rc.1 和 v1.1.0，正式回滚目标为 v1.0.0。
...
9. `700px` 及以下使用单行篇章信息，展开后显示完整难度、章节、书目和分类；桌面侧栏保持完整。

移动端书库筛选还验证：

1. 700px 及以下隐藏平铺筛选栏，显示 `筛选（已选 N 项）` 和当前筛选 chips。
2. 抽屉选择使用草稿，关闭或 Escape 丢弃，确认后一次同步 URL、页码和结果。
3. chips 可单项移除或全部清除；清除时保留清除前滚动位置，内容不足时由浏览器自然钳制。
4. 对话框限制焦点、锁定背景滚动，关闭后把焦点还给筛选按钮。
