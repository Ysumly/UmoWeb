...
- Playwright 62 个浏览器检查，其中 38 个 functional 用例覆盖公开端、隐私说明、在线编辑器、
...
- Windows 本机 Chrome 当前运行 62 个 Playwright 检查，24 张 `win32` 视觉快照通过。
- GitHub Actions Ubuntu 使用 Playwright 1.63.0 的 Chromium 运行同样的 62 个检查，
  通过独立的 `linux` 视觉快照验证。
