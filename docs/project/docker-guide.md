...
该脚本覆盖公开 8 个和管理 23 个兼容接口，并验证 401、改密旧 token 失效、搜索 429、
图片完整生命周期和图片一致性来源定位。默认报告 31/31；显式追加 `-IncludeAI` 后覆盖
9 个 AI 管理接口并报告 40/40。脚本会临时修改管理员密码和 `site_title`，完成后恢复并
清理测试资源。
...
- AI 默认关闭；启用后使用 `AiRequestGuard` 限制单实例同时最多一个请求，不设时间窗口或
  调用次数配额，生产密钥和模型只放在服务器侧。
