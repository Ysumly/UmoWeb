...
| 测试 | Spring Boot Test、Mockito、MockMvc；238 个测试（19 个 MySQL 环境门控） |
...
| 拼音 | pinyin-pro 3.29.x，管理端新建文章标题转 slug |
...
- 管理端 AI 已完成 5.1B 模式目录、5.1C 设置页、5.1D 转换运行时、5.1E 文章 AI 抽屉
  和 5.1F 自动/真实模型质量验收；正式版 `v1.1.0` 已发布。代码部署默认值仍为关闭，
  当前 ECS 生产已显式开启并启用五个默认模式。
...
| 视觉基线 | 36 张 Windows Chrome 与 36 张 Linux Chromium 独立 PNG |
...
- 公开列表 `sort` 仅当值严格等于 `created_at_desc` 时按创建时间倒序，其他值按
  `published_at DESC`。管理端列表先按 `DRAFT`、`SCHEDULED`、`PUBLISHED`、`ARCHIVED`
  分组，再在组内按所选时间字段和 `id DESC` 排序。
...
- 管理端登录页。
- 管理端响应式布局、主题切换和退出登录。
- 管理端文章列表：状态优先级分组、四状态筛选、分页、当前页批量分类/标签、归档/恢复、
  状态展示、整行点击编辑和删除。
- 管理端文章列表与公开列表共享 `includeDescendants` 参数；书库选择分类时默认包含全部子分类，
  URL 同步 `category=<id>&includeDescendants=true`。
- 管理端文章编辑器：新建/编辑、已有文章每 30 秒自动保存、分类可搜索下拉多选、
  标签单行分页、metadata 校验、Markdown 分屏预览、可展开的 metadata 字段说明、
  图片选择/拖拽/粘贴、定时发布和未保存离开保护。自动保存采用最后提交覆盖，不提供版本冲突检测。
  新建页支持单文件 `.md`/`.markdown` 导入，解析 YAML front matter，并在缺失时从 H1、
  文件名和默认值回退。
...
- 管理端文章 AI 抽屉：能力开启时在正文工具栏显示入口，支持正文带入、模式执行/取消、
  ...
  结果源码、预览、复制和持久化共享同一字符串状态；超长结果返回后聚焦结果区并重置内部滚动位置。
...
- 2026-09-19 已完成移动阅读体验增强：管理端新建文章从中文标题转拼音生成 slug，手改后锁定且
  编辑页不自动改写既有 slug；700px 及以下文章详情把篇章信息、难度、分类、章节和书目压缩为
  单行可展开区；书库改用已选数量按钮、当前筛选 chips 和草稿式底部抽屉，确认后一次更新 URL，
  清除筛选保留滚动位置。对应 Windows 和 Linux 移动端书库、文章详情视觉基线已更新；
  Linux 生成工作流为 `35442006114`，其余 32 张快照未变化。

### 6.2 其他前端事实

- 管理端路径由统一 `VITE_ADMIN_PATH` 工具控制，默认 `/secret-admin`，不再依赖后端 `app.admin-path`。
- 当前存在公开端 `SiteHeader`、`SiteFooter`、`ContentCard`、`ContentState`、`LibraryFilterSheet`、`MarkdownArticle`、
  `ThemeToggle`、`GameShell`、`GameResultDialog` 和 `ToolsPage`，管理端新增
  `AdminCategorySelect`；公开导航由
...
- 2026-09-17 已发布正式版 `v1.0.0`：提交 `fe008c59cb71`，发布 CI run `35232962034`；
  ...
  该版本随后由 `v1.1.0` 替代，当前正式回滚目标为 `v1.0.0`。
- 2026-09-19 已发布 `v1.1.0-rc.1`：提交 `bba9e44dc640`，push CI run `35431353455`；
  关闭态独立 `Verify`、ECS 31/31 和 capability 禁用检查通过。
- 2026-09-19 已发布正式版 `v1.1.0`：与 RC 同一提交，正式镜像、独立 `Verify`、ECS 31/31、
  关闭态 capability、公开端和现有管理端检查均通过。真实 DeepSeek 完成一次
  `STRUCTURE_CLEANUP` 受控转换，结构完全一致，日志未发现正文、结果、提示词或 API Key。
  当前生产版本为 `v1.1.0`，开发机保留 `v1.0.0`、`v1.1.0-rc.1` 和 `v1.1.0` 三个归档。
- 2026-09-19 已在当前 ECS 设置 `APP_AI_ENABLED=true` 并启用五个默认转换模式；
  能力接口返回五个模式，输入上限保持 20,000，随后 31/31 兼容冒烟继续通过。
- 2026-09-19 已发布正式版 `v1.1.1`：提交 `e0fe7ea29de4`，push CI run `35442876111`；
  发布前备份、独立 `Verify`、ECS 31/31、公网首页和站点 API 均通过。前后端 image ID 分别为
  `sha256:98d083acb6e8f0e20d68d3a05cc96bd1c40b9b00717c0d777ffdbdbaa5fd59be` 和
  `sha256:f2ab08d87785c70f42d764ef9987151278fbefd7bdb3ecd83d3f91a3056c905f`。
  当前生产版本为 `v1.1.1`，开发机保留 `v1.1.0` 与 `v1.1.1`，正式回滚目标为 `v1.1.0`。
- 2026-09-20 已发布正式版 `v1.1.2`：提交 `02ad7236c258`，push CI run `35495066760`；
  删除 AI 本地 10 分钟 5 次窗口配额并保留单并发。发布前备份、独立 `Verify`、ECS 31/31、
  公网首页和站点 API 均通过；6 次真实 `MODERN_TO_CLASSICAL` 连续转换全部成功且日志无
  `LOCAL_RATE_LIMITED`。该版本随后由 `v1.1.3` 替代，当时正式回滚目标为 `v1.1.1`。
- 2026-09-24 已发布正式版 `v1.1.3`：提交 `0920b0a1b87e`，push CI run `35960680216`；
  管理端文章状态分组、分类下拉、30 秒自动保存、整行编辑和 AI 结果预览修复通过发布前
  备份、独立 `Verify`、ECS 31/31、公网首页与站点 API 检查。当前生产版本为 `v1.1.3`，
  开发机保留 `v1.1.2` 与 `v1.1.3`，正式回滚目标为 `v1.1.2`。
...
- 前端 139 个 Node 测试覆盖路由、管理路径、主题解析、隐私配置、游戏规则与旧成绩解析、
  管理端文章/分类/标签/图片/站点/改密表单规则、API 错误解析、编辑器草稿与文件规则、
  Markdown front matter 导入、日期格式、书库后代参数与筛选 chips、拼音 slug、目录树与展开状态、标题 ID/别名、
  AI 模式表单与版本载荷、AI 抽屉本地状态/字符边界、Markdown 原始 HTML、邻接正文的加粗、
  危险 URL 协议、图片 alt 转义、定时状态、编辑器滚动比例、标题锚点插值和批量载荷规则。
- Playwright 每个平台运行 134 个浏览器检查：98 个 functional 用例覆盖公开端、正文摘要、
  文章目录/阅读进度/相关阅读、移动端篇章信息压缩、书库筛选抽屉与滚动恢复、图片一致性报告与竞态、隐私说明、工具中心、在线编辑器、
  三处编辑工作区双向滚动、标题区间单调插值与缓存失效、批量文章操作、定时发布、桌面目录常驻/侧栏内滚动/窄屏断点、
  四款游戏的高密度/长序列/旧成绩兼容、AI 模式卡片与策略说明、AI 转换抽屉、移动菜单焦点/横屏滚动、
  隐藏文件输入和管理端核心流程；
  36 个视觉断言覆盖 18 个核心页面状态的 `1440×900` 与 `390×844` 基线。
...
- 仓库分别保存 36 张 `win32` 和 36 张 `linux` 视觉快照；Linux 快照通过手动
