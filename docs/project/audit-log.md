...
## 审计 #59 - 2026-09-19 — 5.1F Task 5 自动验收与受控真实模型评测

### 范围

- 在独立本地 MySQL 8.4 和 `deepseek-flash` 上完成五种模式、10 个固定样本的真实模型
  质量评测；输入为仓库内合成样本，不写文章接口，不修改生产数据。
- 补齐假供应商协议、可配置 AI 接口冒烟、全量后端/前端、Windows Playwright、
  Linux AI 视觉基线和运维脚本验收。
- 真实 API Key 只从 Git 忽略的本地评测文件注入临时后端进程；原始输出、
  评测结果和运行日志保存在 Git 忽略的 `Downloads/ai-evaluations/` 中。

### 契约

- 真实评测按 10 个样本执行，单批最多 5 次转换；所有模式使用版本 1 的初始提示词。
- 结构整理以 `EXACT_CONTENT` 自动校验为硬门禁，翻译核对数字、专有名词、链接和代码，
  轻度小说化必须保持在原文 1.0–1.5 倍长度内。
- 真实密钥、系统提示词、样本原文和完整模型输出不进入日志、版本库或审计摘要。
- 首次 `novel-scene` 上游返回无效响应后立即停止，保留前 9 个成功结果；单独重启后端并仅
  重试该样本后通过，没有重跑其他已成功样本。

### 验证

| 验证 | 结果 |
|---|---|
| 后端全量测试 | 238 个，0 失败，18 个 MySQL 门控跳过 |
| MySQL 8.4 集成 | 17 个门控测试 0 失败；Schema、种子和迁移通过 |
| 真实接口冒烟 | 40/40、77 次 HTTP 请求通过，含 AI 模式 CRUD、回滚、能力和转换契约 |
| 前端 Node 测试 | 120/120 通过 |
| Windows Playwright | 110/110 通过，含 76 functional 和 34 visual |
| Linux 视觉基线 | 既有手动工作流生成的 AI 设置与抽屉四张基线已提交 |
| 脚本验收 | 假供应商、评测脚本、敏感信息扫描和 9 个访问安全测试通过 |
| 真实模型评测 | 10/10 成功，全部评分 5/5，模式版本均为 1 |
| 调用与 Token | 11 次调用（10 次成功、1 次无效响应后重试）；11,311 Token |
| 客观边界 | 结构样本正文一致；小说化长度分别为 1.50 和 1.48 倍 |
| 日志边界 | 未发现 API Key、系统提示词或评测样本原文 |
| 差异格式检查 | `git diff --check` 通过 |

### 状态

- `5.1F` Task 5 已完成，质量门禁允许进入文档收口、关闭状态发布和受控启用演练。
- `5.1F` 仍未整体完成，Task 6 的接口/Schema/模块文档同步、生产发布状态记录和总索引更新
  尚未执行；当前生产版本仍为 `v1.0.0`。
- 剩余风险：本次真实评测包含一次可重试的上游无效响应；质量结果来自单模型单轮固定样本，
  费用边界仍由服务器侧账号和供应商计费控制。

## 审计 #58 - 2026-09-18 — 5.1E 管理端文章 AI 抽屉

### 范围

- 在管理端文章编辑器增加仅按能力开关显示的 AI 转换抽屉，支持正文带入、模式选择、
  执行、取消、结果编辑、只读 Markdown 预览、复制、重新转换确认和浮动恢复。
- 源草稿固定保存到 `sessionStorage["umo-admin-ai-source-v1"]`，结果固定保存到
  `localStorage["umo-admin-ai-result-v1"]`；存储异常时降级为当前会话内继续编辑。
- 后端仅移除与 Service code point 校验重复的 Jakarta `@Size` 注解；不修改数据库 Schema、
  文章保存 payload、文章 dirty 状态或 ECS 生产数据，不接入真实 DeepSeek 质量评测。

### 契约

- 入口只在 `/api/admin/ai/capabilities.enabled=true` 时显示；能力查询失败不影响文章编辑器。
- 正文必须由用户点击“带入当前正文”后进入源草稿，按 Unicode code point 限制最多 20,000 字符。
- 请求通过 Axios 转发 `AbortController.signal` 和 185 秒超时；同一抽屉禁止重复提交，
  取消或失败不得覆盖已有结果。
- 结果被人工编辑后重新转换前必须确认；结果只允许编辑、预览和复制，不自动写入文章。
- 429/502/503/504 分别映射限流、无效响应、不可用和超时；有响应消息时保留请求 ID。

### 验证

| 验证 | 结果 |
|---|---|
| AI 抽屉 Node 测试 | 10/10 通过 |
| 前端全量 Node 测试 | `npm test`，120/120 通过 |
| 后端全量测试 | `mvn test`，237 个测试，0 失败，18 跳过 |
| 前端生产构建 | `npm run build` 通过 |
| Windows functional | 76/76 通过，其中 AI 抽屉 9 个流程 |
| Windows visual | 34/34 通过，新增桌面和移动 `admin-ai-drawer` 基线 |
| 复制回退 | Clipboard API 拒绝后使用 `document.execCommand('copy')`，浏览器用例通过 |
| 远程图片隔离 | AI 预览不生成远程 `<img>`，第三方图片请求为 0 |
| 离开保护 | 请求中浏览器返回会确认；取消后仍停留在编辑器 |
| 差异格式检查 | `git diff --check` 通过，仅有 Windows LF/CRLF 提示 |

### 状态

- `5.1E` 标记为已完成；`5.1F` 继续负责假供应商回归、受控真实模型评测、Linux 视觉基线、
  真实 HTTP 冒烟和发布门禁。
- 当前生产版本仍为 `v1.0.0`；本次不触发 ECS 发布，不修改生产数据。
- 剩余风险：真实模型质量尚未评测，来源草稿会发送到已配置的第三方 AI 服务，
  关闭标签页后本地草稿不保证保留。

## 审计 #57 - 2026-09-18 — 5.1D 管理端 AI 转换运行时

### 范围

- 新增 AI 设置、能力查询和转换接口，以及 32 个管理端接口的统一计数基线。
- 新增供应商无关 Provider、DeepSeek OpenAI-compatible 适配、180 秒超时、请求窗口、
  并发控制、四类结果校验、错误归一化和脱敏元数据日志。
- 移除未使用的 Spring AI BOM、OpenAI Starter 和 `spring.ai.openai` 配置；
  不修改数据库 Schema、文章编辑器、公开端或 ECS 生产数据。

### 契约

- `APP_AI_ENABLED=false` 时应用正常启动；启用时 `DEEPSEEK_API_KEY` 和
  `DEEPSEEK_MODEL` 必填，限制值必须为正数。
- 转换输入最多 20000 字符，输出最多 60000 字符；10 分钟最多 5 次且同时最多 1 个请求。
- 模式不存在返回 404，模式停用返回 409，本地限流返回 429；Provider 错误映射为
  `502/503/504`，不返回上游响应体。
- 日志只记录请求 ID、模式、版本、字符数、Token、耗时、状态和错误分类；
  正文、结果、系统提示词和 API Key 不进入日志，结果不自动写入文章。
- 限流状态为单实例内存实现，多实例部署需要共享存储或网关限流。

### 验证

| 验证 | 结果 |
|---|---|
| 后端全量测试 | `mvn test`，236 个测试，0 失败，0 错误，18 跳过 |
| AI 定向测试 | 配置、Provider、限流、结果校验、Service、Controller 和边界测试通过 |
| 依赖检查 | 过滤 `org.springframework.ai:*` 的 dependency tree 无输出 |
| 敏感信息扫描 | `bash scripts/ci/scan-sensitive-info.sh` 通过 |
| 差异格式检查 | `git diff --check` 通过，仅有 Windows LF/CRLF 提示 |
| 真实 MySQL | 由既有 17 个 CI 环境门控覆盖；本次不新增 Schema |

### 状态

- `5.1D` 标记为已完成，`5.1E` 可以消费能力查询和转换接口，`5.1F` 继续负责真实 HTTP
  冒烟、受控真实模型评测和发布。
- 当前生产版本仍为 `v1.0.0`；本次不触发 ECS 发布，不修改生产数据。
- 剩余风险：真实 DeepSeek 质量和费用表现尚未评测，单实例限流不适用于多后端实例。

## 审计 #56 - 2026-09-18 — 5.1C 管理端 AI 模式设置页

### 范围

- 新增管理端 AI 设置路由、侧栏入口和响应式页面，支持模式新建、复制、编辑、启停、排序、
  历史提示词预览和回滚。
- 新增六个模式目录 API 封装和纯函数表单/校验/载荷模块；更新使用 `expectedVersion`，
  409 后保留本地表单并支持重新加载。
- 不修改后端、数据库 Schema、文章编辑器和转换运行时，不接入真实模型，不新增前端依赖。

### 契约

- 页面只消费 5.1B 六个冻结接口，路由为 `adminPath('ai-settings')`，不写死管理端前缀。
- 新建和复制默认停用；`modeKey` 创建或复制时必填，编辑时只读。
- 提示词或校验策略变化由后端生成新版本；元数据、启停和排序变化不增加版本。
- 历史版本只读预览；回滚前确认并生成新版本，不覆盖历史。

### 验证

| 验证 | 结果 |
|---|---|
| 前端 Node 测试 | `npm test`，109/109 通过 |
| 前端生产构建 | `npm run build` 通过 |
| Windows Playwright | 99/99 通过，含 67 functional 和 32 visual |
| 新增功能测试 | 创建、复制、编辑、启停、排序、版本预览、回滚、409 和移动端单列通过 |
| 视觉回归 | 新增 AI 设置桌面与移动 `win32` 基线；既有 30 张基线未变化 |
| 差异格式检查 | `git diff --check` 通过 |

### 状态

- `5.1C` 标记为已完成；`5.1D` 和 `5.1E` 保持待实施。
- Linux 下 AI 设置的两张视觉基线由 `5.1F` 统一生成和审查。
- 当前生产版本仍为 `v1.0.0`；本次不触发 ECS 发布，不修改生产数据。

## 审计 #55 - 2026-09-18 — 5.1B 管理端 AI 模式目录后端

### 范围

- 新增 AI 模式与不可变提示词版本的数据库结构、幂等迁移、默认模式和管理端六个 API。
- 实现模式列表、新建、复制、元数据/提示词更新、最近 10 版保留和历史回滚。
- 不接入供应商模型，不新增前端页面，不部署 ECS，不把提示词写入 Java、Vue 或测试常量。

### 契约

- `modeKey` 唯一且创建后不可修改；创建和复制从 version 1 开始，复制模式默认停用。
- 提示词或校验策略变化生成新版本；名称、说明、启停和排序变化不生成版本。
- `expectedVersion` 过期返回 409；回滚复制历史版本为新版本，不改写历史。
- 运行时只通过 `AiModeCatalogService.findEnabledByKey(...)` 读取启用模式当前版本。
...
- `5.1B` 标记为已完成，`5.1C` 和 `5.1D` 的依赖已解除并保持待实施。
- 当前生产版本仍为 `v1.0.0`；本次不触发 ECS 发布，不修改生产数据。

## 审计 #54 - 2026-09-18 — 5.1A 管理端 AI 产品与数据契约

### 范围

- 冻结管理端 AI 的产品边界、隐私规则、模式、接口、字段、版本策略和本地存储契约。
- 将 Post-v1、前端规格、前端架构、总 SRS、状态快照和代码基线统一标记为
  “计划已拆分、运行时代码未实现”。
- 不修改 Java、Vue、数据库 Schema、运行时配置或真实模型供应商。

### 契约

- 五个 `modeKey`、四个 `validationProfile`、九个管理端接口、四个环境变量和两个浏览器
  存储键已由总索引与 `5.1A` 固定。
- 模式支持新增、停用、复制、排序、乐观锁更新和最近 10 版回滚；首版不提供永久删除。
- 原始草稿使用 `sessionStorage["umo-admin-ai-source-v1"]`，结果使用
  `localStorage["umo-admin-ai-result-v1"]`，不进入数据库或服务器备份。
...
- `5.1A` 标记为已完成，`5.1B` 的前置依赖已解除但保持待实施。
- 当前生产版本仍为 `v1.0.0`，正式回滚目标为 `v1.0.0-rc.8`；本次不触发 ECS 发布。
...
- 将文章目录常驻与邻接 Markdown 加粗修复同步到 ECS，发布版本 `v1.0.0-rc.8`。
- 不修改后端 API、数据库 Schema、Markdown 存储格式、认证或部署拓扑。
...
|---|---|
| 发布版本 | `v1.0.0-rc.8` |
| Git commit | `12bc8111cf36676a894942f06b1955cccbbfedb4` |
| Push CI | run `35231477064`，五个 job 全部成功 |
| 后端 image ID | `sha256:3f1bdf43c4eea8308d34e2c25c25c10ca0de105a070a4875f7ce236a34f69f53` |
| 前端 image ID | `sha256:afa918d2ece757c35349a72482784929770559fd5c7d7a06015881963bb5f0a9` |
| 发布归档 | 175685632 字节，SHA-256 `137405c8e6e5927169cc17d4dfd52ed3c8c0e27e10ddc617c131283dbc3080e5` |

### 验收

- annotated tag `v1.0.0-rc.8` 已推送并确认 peel 后指向发布 commit。
- 独立 `Verify` 返回 rc.8、manifest 双镜像 ID 和 `operation=deploy`。
- ECS 后端启动后回填 29 篇已发布正文；`api-smoke.py` 31/31 通过，覆盖认证保护、
  草稿隔离、搜索限流、图片生命周期、图片一致性、定时隔离和批量操作。
- 公网浏览器在 `1219×958` 下确认目录滚动前后均可见、trigger 隐藏，目标列表 `strong`
  计算字重为 `700`，正文无字面星号且控制台无页面错误。

### 边界与回滚

- 当前生产版本为 `v1.0.0-rc.8`，正式回滚目标为 `v1.0.0-rc.7`；开发机只保留这两个归档。
- 本次没有新增 SQL 迁移，发布仍执行现有幂等迁移、正文索引回填、运行时和访问服务检查。

## 审计 #51 - 2026-09-17 — 文章目录常驻与邻接 Markdown 加粗修复

### 范围

- 调整公开文章目录：桌面始终保留侧栏目录，只在 `980px` 及以下使用悬浮目录。
- 修复无序列表中 `**内容，**紧接正文` 被 CommonMark 保留为字面星号的问题。
- 不修改后端 API、数据库 Schema、Markdown 存储格式、认证或编辑器行为。

### 实现

- 删除按目录自然高度切换桌面抽屉的 `shouldUseOutlineDrawer`、`ResizeObserver` 测量和相关
  trigger/panel 状态；侧栏继续使用 `50vh` 高度上限与内部滚动。
- `ArticleOutline` 只保留桌面侧栏和窄屏悬浮目录两种响应式形态；`981px` 以上目录常驻，
  `980px` 以下显示原有打开、关闭、焦点恢复和外部点击行为。
- marked 增加 inline `adjacentStrong` 扩展，在加粗结束符后紧接正文且标准分词无法闭合时
  生成 `strong` token；代码块、代码行和既有安全渲染路径保持不变。
...
| 生产构建 | Vite 8.1.0 通过 |
| 回归断言 | 桌面常驻/侧栏内滚动/981–980px 断点均通过 |

### 剩余风险

1. 邻接加粗扩展只放宽同一行内结束符后无空白的写法，不改变 CommonMark 对换行、代码和链接的
   其它解析规则。
