...
- 处理 2026-09-21 归档的 6 条管理端使用反馈：分类换行/简写、自动保存、状态顺序、
  整行编辑和 AI 结果源码与预览不一致。
- 不修改数据库 Schema、公开端接口、AI 请求协议或生产 ECS 数据。

### 实现

- 管理端列表使用独立 SQL 排序片段，固定按 `DRAFT → SCHEDULED → PUBLISHED → ARCHIVED`
  分组，再按所选时间字段和 `id DESC` 排序；公开端继续使用原排序片段。
- 文章编辑器分类改为可搜索下拉多选面板，完整显示分类名并保留父子缩进；分类不再分页，
  标签保持原单行分页。
- 编辑已有文章每 30 秒在内容变脏、校验通过且无并发操作时自动保存完整表单；失败保留修改并
  在下一周期重试，随后在页头显示成功/失败/待修正状态。新建文章不自动创建。
- 点击文章行非交互区域进入编辑页，复选框、编辑链接和删除按钮保持原行为。
- AI 结果源码、预览、复制和本地持久化共享顶层字符串状态；结果返回后聚焦源码并重置选中范围
  和内部滚动位置。桌面长结果曾因原生 `textarea` 未稳定绘制且保留旧滚动状态而显示空白，
  以长内容视觉和浏览器回归固定修复。
...
| 验证 | 结果 |
|---|---|
| 后端单元测试 | 238 个测试通过，0 失败、0 错误，19 个真实 MySQL 测试按环境门控跳过 |
...
- 不修改 API 请求/响应形状、数据库列、Markdown 文件格式或生产配置。
- 自动保存采用用户选择的最后保存者覆盖策略，不提供版本冲突检测或字段合并。

## 审计 #69 - 2026-09-20 — 修复生产 AI 模式校验策略不匹配

### 范围

- 用户请求 `dd45c38e-19ab-4545-b07b-128af6420fd2` 的
  `STRUCTURE_CLEANUP` 转换返回「AI 服务返回无效响应」。
- 只调整生产 AI 模式目录数据，不修改代码、API、Schema、镜像或发布版本。

### 根因

- 生产 `STRUCTURE_CLEANUP` 当前版本为 `2`，提示词明确允许错别字、语法、病句、
  重复表达、标点和语言流畅度优化。
- 同一版本的校验策略仍为 `EXACT_CONTENT`，要求除标题外正文字符完全一致；模型按提示词
  优化措辞后会被本地结果校验拒绝，并统一显示为 502「AI 服务返回无效响应」。
- 最小复现请求 `8f3ce1e2-2f9e-4a75-ba7a-1a46a1d4bfd7` 明确返回
  `结构整理改变了正文`，确认不是 DeepSeek JSON 损坏或上游空响应。

### 修复与验证

- 通过管理 API 保留版本 2 的提示词与启用状态，仅将校验策略改为 `NONE`，生成版本 `3`；
  版本 1、2 历史仍可查询和回滚。
- 当前生产模式状态为 `STRUCTURE_CLEANUP`、`current_version=3`、`validation_profile=NONE`、
  `enabled=1`。
- 使用同一复现输入再次调用，请求 `6a22035e-7328-4236-a78e-d8f48adf86dc` 返回 200，
  `modeVersion=3`、`status=success`，对话日志无正文、结果、提示词或 API Key。

### 状态

- 生产继续运行 `v1.1.2`，无需重新发布或回滚。
- 五个默认模式保持启用；本次只修复 `STRUCTURE_CLEANUP` 的运行时校验配置。

## 审计 #68 - 2026-09-20 — 移除 AI 请求窗口限流并发布 `v1.1.2`

### 范围

- 修复线上 AI 请求 `5f8830c1-06fa-496f-9f28-affb0d94e46d` 的本地
  `LOCAL_RATE_LIMITED`；根因为 10 分钟最多 5 次的进程内窗口配额。
- 删除窗口计数和对应配置，仅保留同时最多 1 个 AI 请求；不修改 API、Schema、前端交互或
  DeepSeek 上游错误映射。
- 通过 PR #35 合并提交 `02ad7236c2584a2691e0ad9688c2efdd124c7695`，并发布正式版 `v1.1.2`。

### 契约与验证

| 验证 | 结果 |
|---|---|
| Push CI | run `35495066760`，五个 job 全部成功 |
| 后端测试 | 237 个测试通过，0 失败、0 错误，18 个 MySQL 门控测试按预期跳过 |
| 窗口边界回归 | 连续 6 次以完成并释放许可的调用全部通过，不再出现第 6 次本地 429 |
| 单并发回归 | 第一个调用持有许可时第二个调用返回 429，结束后可继续调用 |
| 发布前备份 | `20260920T064919Z`，SHA-256 `c12231370f0c167ce79c0452ab9d5e11ddbcb554f4ab42449c8486e6707f6f7d` |
| 后端镜像 | `umoweb-backend:v1.1.2`，image ID `sha256:89bc5381e73ff1b802f7f955d86335fa5ae0079f7e37ee66b947632959c6a971` |
| 前端镜像 | `umoweb-frontend:v1.1.2`，image ID `sha256:f33197d239746e6478f0e8ce39d8b29af0424d2f7dcf082f60340326b7dbd539` |
| 镜像归档 | SHA-256 `ef67e5f8e07e2b6b640a4fe8e6252b21cd17037959bb1adf08b2625e6a88b29d`，147810816 字节 |
| Git 标签 | annotated `v1.1.2` peeled to `02ad7236c2584a2691e0ad9688c2efdd124c7695` |

### 生产验收

- 独立 `Verify` 返回当前 release `v1.1.2`、manifest 双镜像 ID、`operation=deploy`，访问日志、
  隐私配置、权限和回环报表检查通过。
- ECS `api-smoke.py` 为 31/31；公网首页与公开站点信息均返回 200。
- 在 10 秒内完成 6 次真实 `MODERN_TO_CLASSICAL` 转换，全部 HTTP 200；后端日志均为
  `status=success`，没有 `LOCAL_RATE_LIMITED`，证明原 5 次窗口不再拦截后续请求。
- 请求 ID：`c9679219-c2b4-4e08-98b5-4698260dc4ac`、`fd777ec5-cd80-4365-9ae2-7e7f6e50a9ef`、
  `21bf8468-e8af-4495-9d58-5e9901235344`、`102fb13c-4833-467c-bb39-86325b4181f0`、
  `1b96826a-4229-402b-a66a-58a59133f0b2`、`7927a280-e781-4d45-ad91-78740a9c1864`。

### 状态

- 生产当前运行 `v1.1.2`，正式回滚目标为 `v1.1.1`。
- 开发机保留 `v1.1.1`、`v1.1.2` 两个归档；ECS 不长期保存旧归档。
- 生产 `APP_AI_ENABLED=true` 和五个默认模式状态保持不变。

## 审计 #67 - 2026-09-19 — 移动阅读体验正式版 `v1.1.1`

### 范围

- 将 PR #34 的移动阅读增强合并到 `master`，合并提交为 `e0fe7ea29de4`。
- 通过现有本地版本化发布链路构建、上传、迁移、切换和验收 `v1.1.1`。
- 不修改 API、Schema、生产内容、AI 模式或访问策略。

### 发布证据

| 项 | 结果 |
|---|---|
| Push CI | run `35442876111`，五个 job 全部成功 |
| 发布前备份 | `20260919T123016Z`，SHA-256 `57f526b3340414b00ed6510e958b9a524c532d06e4e4defb34eefbce4918373f` |
| 后端镜像 | `umoweb-backend:v1.1.1`，image ID `sha256:98d083acb6e8f0e20d68d3a05cc96bd1c40b9b00717c0d777ffdbdbaa5fd59be` |
| 前端镜像 | `umoweb-frontend:v1.1.1`，image ID `sha256:f2ab08d87785c70f42d764ef9987151278fbefd7bdb3ecd83d3f91a3056c905f` |
| 镜像归档 | SHA-256 `f9865bf857a137333fb0d8f6e9c83190d1b9003105cd0afe79c30c6804094a7f`，147811328 字节 |
| Git 标签 | annotated `v1.1.1` peeled to `e0fe7ea29de41b7ef996424f87eeae60691646cb` |

### 验收

- 独立 `Verify` 返回当前 release `v1.1.1`、manifest 双镜像 ID 和 `operation=deploy`。
- ECS 内 `api-smoke.py` 为 31/31，覆盖认证、草稿隔离、筛选、关联、前后篇、相关阅读、
  改密旧 token 失效、图片生命周期、调度隔离、批量操作和搜索限流。
- 公网首页返回 200；公开站点信息返回 200 且响应包含正式站点配置。
- 发布脚本完成镜像载入、幂等迁移、容器切换、正文索引回填和运行健康检查，无需回滚。

### 状态

- 生产当前运行 `v1.1.1`，正式回滚目标为 `v1.1.0`。
- 开发机保留 `v1.1.0`、`v1.1.1` 两个归档；ECS 不长期保存旧归档。
- 生产 `APP_AI_ENABLED=true` 和五个默认模式状态保持不变。

## 审计 #66 - 2026-09-19 — 移动阅读体验增强

### 范围

- 新建文章增加中文标题拼音 slug、手动锁定和导入 slug 保护，不修改后端接口或数据库。
- 压缩 `700px` 及以下文章详情篇章信息，桌面侧栏和阅读工具保持原行为。
- 移动端书库改为草稿式筛选抽屉、当前筛选 chips、单项移除和清除后滚动恢复。
- 同步前端架构、规格、测试指南、代码基线记忆和状态快照；历史发布记录不重写。

### 契约

- slug 只由前端生成，继续遵守后端 `[A-Za-z0-9][A-Za-z0-9_-]{0,199}` 约束；
  用户手改后标题不再覆盖，编辑页清空 slug 不自动生成，重复值仍由 409 提示处理。
- 书库移动筛选只在 `700px` 及以下替换平铺筛选栏；草稿通过确认按钮应用，关闭或 Escape 丢弃。
- 清除筛选先记录 `scrollY`，结果返回后恢复；内容不足时由浏览器自然钳制，不主动回顶。
- 抽屉使用 `role="dialog"`、`aria-modal`、焦点圈定和背景滚动锁定，关闭后焦点返回筛选按钮。
...
- 将 5.1F 文档收口提交 `b0a7edb` 和 Linux 移动端断言稳定性修复提交 `0e26198`
  通过 PR #33 合入 `master`，合并提交为 `bba9e44dc640`。
- 发布 AI 默认关闭的 `v1.1.0-rc.1` 和正式版 `v1.1.0`；不修改运行时代码、API、
  Schema 或真实内容。
...
| 项 | 结果 |
|---|---|
| Push CI | run `35431353455`，五个 job 全部成功 |
| RC 构建提交 | `bba9e44dc640` |
| 正式版构建提交 | `bba9e44dc640` |
| RC 后端 image ID | `sha256:d65ddcb290b64f7e7e0bb152df405bab2e6f13dfcc43c6f3f88d09e6eec7c1e1` |
| RC 前端 image ID | `sha256:8abe2c566f9dd85fe4bca8076f1349618d3cc91148139c70a74f68cec257ddd6` |
| 正式版后端 image ID | `sha256:43466dcd60345f79057cb02b363f2ef3f71ec2758f922476d469b11f18bf8db6` |
| 正式版前端 image ID | `sha256:b0ee17bb1b9aad6f6e363fbc3d8b56f7e20e888365ee42c109e232b01a7d706a` |
| RC 归档 SHA-256 | `1f35f707db4dfa6ec5b305459fae1c2e44f0aedeb3de786ec2dd81f4c73616e9` |
| 正式版归档 SHA-256 | `d84f32fe7be6051ac66bbe27f68439896123d59e4c33b73b976370443b8a07a3` |
| 迁移前备份 | `20260919T081631Z`，SHA-256 `98ab9f0130a850a126fdf3b35906cf596002ecea16328660a7bd63c8a0a56271` |

### 验收

- PR #33 的后端 238 个测试、MySQL 8.4 迁移与 40/40 AI 契约冒烟、前端 126 个 Node 测试、
  生产构建和 Linux Playwright 全部通过。
- 关闭态 RC 与正式版均通过独立 `Verify`、ECS 31/31 和 capability `enabled=false` 检查；
  三容器健康，公开端和现有管理端行为未改变。
- 临时启用 `STRUCTURE_CLEANUP` 后完成一次真实转换，模式版本保持 1，
  Token 使用 `102/906/1008`，耗时 `5342ms`，结构与原文完全一致。
- 后端日志未发现合成正文标记或真实 API Key；演练后模式停用并恢复
  `APP_AI_ENABLED=false`，再次完成 31/31。
- annotated tag `v1.1.0-rc.1` 和 `v1.1.0` 已推送，peel 后均指向 `bba9e44dc640`。
- 开发机保留 `v1.0.0`、`v1.1.0-rc.1` 和 `v1.1.0`，正式回滚目标为 `v1.0.0`。

### 状态

- `5.1F` 与整个管理端 AI 计划已完成；生产当前运行 `v1.1.0`，AI 默认关闭。
- 剩余风险：单个后端实例内的请求限流不适用于水平扩展；费用仍由供应商账号侧控制。

## 审计 #63 - 2026-09-19 — 管理端编辑器标题间平滑同步

### 范围

- 修正管理端 Markdown 编辑器的同步滚动：同一标题区间内按位置连续插值，不再直接跳到预览标题顶部。
- 保持同步滚动始终开启，不恢复开关；公开编辑器、About/Project 编辑器、后端、Schema 和 ECS 不变。
- 使用无缓存本机 Docker 全栈和当前 171,982 字符文章完成真实性能验收。

### 契约

- 编辑器标题测量位置与预览标题位置组成锚点序列，并在首尾补齐 `(0, 0)` 和最大滚动位置。
- 滚动时二分查找相邻标题区间，按区间内比例计算另一侧 `scrollTop`，双向使用同一映射。
- 每帧最多更新一次并抑制程序化滚动回环；无标题或两侧标题数量不一致时回退到整体比例同步。
- 标题解析、编辑器镜像测量和预览标题位置继续缓存；仅在正文或宽度变化后重新计算。
- 移动端 `701px` 以下维持单面板，不执行同步，也不引入 CSS 平滑滚动动画。

### 验证

| 验证 | 结果 |
|---|---|
| 前端 Node 测试 | 126/126 通过，新增相等/不等高度区间、首尾锚点和空锚点断言 |
| Windows Playwright | 120/120 通过，含 86 functional 和 34 visual |
| 标题区间回归 | 同一 H2 的 0%、25%、50%、75%、100% 五个位置严格单调 |
| 标题边界对齐 | 起止两个标题相对预览顶部误差约 `0.11px`、`0.28px` |
| 反向同步 | 预览中间位置映射回对应编辑器区间，不跳回标题顶部 |
| 缓存失效 | 正文变化和容器宽度变化后仍保持连续映射 |
| 172KB 文章性能 | 90 帧平均 `11.14ms`、P95 `12.6ms`、最大 `16.7ms` |
| 本机全栈 | 无缓存前端镜像部署到 `http://localhost:18080` 后复验 |
| 生产构建 | `npm run build` 通过 |

### 状态

- 管理端编辑器标题间平滑同步已完成，当前生产版本仍为 `v1.0.0`。
- 本轮未修改 API、数据库、AI 逻辑、生产数据或 ECS；发布继续等待本修复提交的 push CI 成功。
- 同标题内部若编辑器换行与 Markdown 渲染换行差异过大，正文仍可能有轻微内部偏差，但标题边界
  精确对齐且滚动过程连续。

## 审计 #62 - 2026-09-19 — AI 工作流布局与滚动体验修复

### 范围

- 调整 AI 模式卡片的状态、启停操作位置和整卡点击边界。
- 统一启用状态与校验策略控件的对齐和高度。
- 将 AI 设置操作按钮与历史版本移动到左侧模式列表下方。
- 强化 AI 抽屉模式选择框的控件视觉。
- 恢复管理端文章编辑器默认同步滚动，移除同步开关，并优化标题锚点同步性能。

### 契约

- 模式卡片右上角只展示当前启停状态，左下角提供独立启停操作；主区域点击选择模式。
- 启用状态与校验策略控件在策略说明展开前后均保持相同顶部位置和高度。
- 操作按钮和历史版本属于左侧模式工作区，不随右侧提示词编辑区下移。
- 管理端文章编辑器始终以当前最近标题为锚点对齐两侧，无标题时才回退到整体比例同步。
- 公开在线编辑器和 About/Project 编辑器继续默认同步，不改变原行为。

### 验证

| 验证 | 结果 |
|---|---|
| 前端 Node 测试 | 122/122 通过 |
| Windows Playwright | 119/119 通过，含 85 functional 和 34 visual |
| Linux Playwright | 119/119 通过，run `35424281268` |
| AI 设置专项 | 状态位置、整卡选择、启用控件对齐、左栏按钮与历史版本全部通过 |
| 文章编辑器滚动 | 默认标题锚点对齐、无同步开关和缓存路径通过 |
| 生产构建 | `npm run build` 通过 |
| Windows/Linux 视觉基线 | AI 设置和 AI 抽屉桌面/移动共八张快照更新并人工检查 |

### 状态

- 本轮只修改管理端 AI 与文章编辑器前端交互，不修改 API、Schema、AI 请求结构或 ECS。
- Linux 的 AI 设置和 AI 抽屉四张快照已通过手动工作流重新生成并人工检查。

## 审计 #61 - 2026-09-19 — AI 模式设置可用性修复

### 范围

- 重新设计 AI 设置页的模式目录，消除桌面和移动端必须横向滚动才能看到完整信息的问题。
- 让模式卡片主区域整体可点击，同时保留独立启停按钮。
- 补充模式说明、系统提示词的用途边界和四种校验策略的展开说明。
- 不修改后端接口、数据库 Schema、AI 运行时或 ECS。

### 契约

- 模式说明只用于管理端识别和说明，不进入供应商请求。
- 系统提示词作为 `system prompt` 发送给大模型，界面必须明确提示不要填写敏感信息。
- 校验策略只用于服务器端结果校验，不改变供应商请求正文。
- 模式卡片主区域点击等价于选择模式；独立启停按钮沿用原有保存和 409 冲突处理。
...
- 修复管理端移动侧栏的焦点泄漏、无焦点限制、Escape 缺失和低高度横屏不可滚动问题。
- 修复管理端及公开端移动菜单点击当前页导航项不关闭、公开按钮可访问性状态不完整的问题。
- 将隐藏文件输入移出 Tab 顺序，并为图片加载失败增加无障碍占位。
- 不修改后端 API、数据库 Schema、AI 契约、生产数据和 ECS。

### 契约

- 管理端侧栏关闭时不可聚焦；打开时背景内容 `inert`，Tab 限制在侧栏，Escape 关闭并恢复焦点。
- 低高度横屏使用侧栏内部滚动，不改变桌面固定侧栏和页面主内容布局。
- 图片加载失败仅改变前端展示，不删除图片记录、不修改文件，也不把文件存在等同于内容可解码。
...
|---|---|
| 前端 Node 测试 | 120/120 通过 |
| Windows Playwright | 117/117 通过，含 83 functional 和 34 visual |
| 新增 functional 回归 | 7/7 通过 |
| 生产构建 | `npm run build` 通过 |
| 横屏稳定性 | 低高度用例连续 3 次通过 |
| 本机 Docker 全栈 | 320×568、390×844、667×320、844×390、1440×900 真实界面复验通过 |
| 视觉基线 | 既有 Windows 34 张基线无需更新 |

### 状态

- 发布前前端缺陷修复已完成，未改变公开 API、Schema、AI 运行时或生产环境。
- 当前生产版本仍为 `v1.0.0`；ECS 发布继续等待本修复提交的 push CI 成功。
- 剩余风险：后端上传校验只验证文件签名，图片一致性扫描只验证文件存在性；本地已确认一个
  PNG 签名有效但内容不可解码的文件会被扫描报告视为正常，前端现在会显示错误占位。
