...
| 前端 Node 测试 | 55 tests / 0 failures |
| Windows Playwright | 38 passed（24 functional + 14 visual） |
| 前端生产构建 | Vite 8.1.0 通过 |
| 导入单测 | front matter、回退、枚举、别名冲突、未知分类、图片警告、非法 YAML 和文件边界 |
| 导入浏览器流程 | 有效导入、表单预填、预览、创建、非法 YAML 保留和重复 slug 修正 |
| 敏感信息扫描 | 通过 |
| 敏感信息扫描器自测 | 通过 |
