...
| 新建文章 | `/secret-admin/contents/new` | 元信息、Markdown 导入、图片上传、AI 转换抽屉 | 已接入真实 API |
...
管理端 AI 模式目录、转换运行时和文章抽屉已完成端到端接入，质量验收已通过，发布收口正在进行：
...
| 管理端 AI 质量与发布 | 自动验收和真实模型评测已通过，正在执行发布门禁 | 假供应商回归、固定样本评测和 Linux 基线均已完成 |
...
当前已有 126 个 Node 内置测试；现代 Playwright 套件包含每个平台 120 个浏览器检查
（86 functional + 34 visual）。尚无组件级单元测试框架。
