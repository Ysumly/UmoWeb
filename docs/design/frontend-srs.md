...
- 公开编辑器和 About/Project 桌面分栏按可滚动比例双向同步；管理端文章编辑器默认关闭同步，
  勾选“同步滚动”后按最近标题锚点对齐；390px 单面板不执行同步。
...
当前已有 122 个 Node 内置测试；现代 Playwright 套件包含每个平台 119 个浏览器检查
（85 functional + 34 visual）。尚无组件级单元测试框架。
