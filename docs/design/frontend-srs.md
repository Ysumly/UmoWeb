...
| 文章详情 | `/post/:slug` | Markdown、代码高亮、元信息、目录、章节高亮、阅读进度 | 已接入真实 API |
...
当前已有 82 个 Node 内置测试；现代 Playwright 套件包含每个平台 74 个浏览器检查
（46 functional + 28 visual）。尚无组件级单元测试框架。
