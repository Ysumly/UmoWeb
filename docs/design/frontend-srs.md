...
> 当前版本: 公开端、在线编辑器、四款训练游戏和管理端核心业务页均已实现
...
- Stroop：84 试次、25% 一致试次、五色按钮和数字键 1–5，保存最佳正确率及正确反应时。
...
- visual-desktop 和 visual-mobile 保存 24 张核心页面截图基线。
- 浏览器套件使用 Mock API，本机 Chrome 和 Windows 基线；真实后端继续由 MySQL 冒烟脚本验证。

当前已有 72 个 Node 内置测试；现代 Playwright 套件包含每个平台 62 个浏览器检查
（38 functional + 24 visual）。尚无组件级单元测试框架。
