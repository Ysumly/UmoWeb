...
> 当前版本: 公开端业务页面和管理端文章工作流已接入真实 API
...
修改密码接口和前端 API 函数均已存在，但未提供页面和入口。
...
- 公开端在线编辑器、分类管理、标签管理、站点设置和修改密码页面仍未实现。
...
- 分类、标签、站点设置和修改密码页面的完整操作流。
- 公开在线 Markdown 编辑器。
- 管理端剩余页面的 429、404、409、413 等错误态。
- 可重复执行的桌面和移动端视觉回归。

当前已有 29 个 Node 内置测试覆盖路由、管理路径、主题逻辑、管理端文章表单规则、静态筛选和 Markdown 安全边界；尚无组件测试或浏览器 E2E。
