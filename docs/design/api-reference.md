...
| 429 | 搜索同 IP 10 秒内重复请求、登录失败过多、AI 并发冲突或 DeepSeek 上游限流 |
| 502 | AI 上游请求或响应无效 |
...
输入最多 20000 字符，结果最多 60000 字符，上游超时 180 秒；本地不设时间窗口或调用次数
配额，只限制同时最多 1 个请求。全局关闭或模式停用返回 409，模式不存在返回 404，
本地并发冲突或上游限流返回 429，无效上游请求或结果返回 502，认证、余额或服务不可用返回 503，
超时返回 504。
