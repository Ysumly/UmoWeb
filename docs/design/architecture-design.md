...
- 首页、文章列表、搜索、详情、About、Project。
- Markdown 在线编辑器。
- 文章管理表单、分类/标签管理、站点设置。
- Markdown 渲染、代码高亮、图片拖入/粘贴。

因此，当前版本是“后端 API 基本完成、前端基础设施完成、前端业务页面待开发”的状态。

---

## 9. 已知架构风险

1. 分类和标签删除保护使用了错误的关联查询方向。
2. 文件写入与数据库事务不具原子性。
3. 小说书级目录取自第一个分类 slug，缺少层级语义校验。
4. `page` 和 `size` 没有边界校验。
5. 数据库没有外键，关联数据可能成为孤儿。
6. 限流 IP 记录没有清理，且信任 `X-Forwarded-For`。
